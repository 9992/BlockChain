# Server.py 에서 /Mine 부분에 실행시간 측정을 통해 비교값을 출력 
- 난이도는 6정도를 사용할 예정이다.
# (추가) DB 구조 다시 생각하기
1. 테이블을 여러 개 생성하여 각 노드별로 분배하기
2. 한 개의 테이블에 여러 개의 데이터를 생성하여, 진행하기
3. 검증된 블록에 대해서만 생성하고 동기화 시키고, 체인 변경시 모든 내용 갈아엎는 방식
    (가장 어렵고 무식한 방식)
# 파이썬 DB 연결하여 데이터 저장하기(19.04.04 진행 예정)
# 블록체인의 정보에서 트랜잭션의 민감 내용 수정하기(19.04.04 진행 예정)
# (추가) 리액트 - AWS RDS(DB) 연결하여 데이터 불러오기 (19.04.05 진행 예정)
